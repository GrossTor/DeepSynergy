...
if not 'X' in locals():
    ...
    file = gzip.open(os.path.join(DATA_FOLDER,'X.p.gz'), 'rb')
    X = pickle.load(file)
    file.close()
...
labels = pd.read_csv(os.path.join(DATA_FOLDER,'labels.csv'), index_col=0) 
...
def split_data(norm = 'tanh', test_fold = 0, val_fold = 1):
